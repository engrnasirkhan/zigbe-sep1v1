...
extern volatile CURRENT_PACKET       currentPacket;
...
extern ROUTE_DST_INFO       routeDstInfo[ROUTE_DST_INFO_SIZE];
    ...
    extern unsigned int     RxRead;
#else
    extern volatile BYTE    RxWrite;
    extern BYTE             RxRead;
#endif

extern BYTE InterruptRoutineSet;
 ...
 ********************************************************************/
 ...
 #if (RX_BUFFER_SIZE > 256)
	#define MBUFFER_CAST WORD
#else
	#define MBUFFER_CAST BYTE
#endif
...
BYTE PHYGet(void)
{
    ...
    if(RxRead == (MBUFFER_CAST)RX_BUFFER_SIZE)
    {
        RxRead = 0;
    }
...
void PHYInit(void)
{
        ...
        if(NOW_I_AM_A_FFD()){//#ifdef I_AM_FFD
        ...
        /* Configure to use external PA LNA */
                ...
                PHYSetShortRAMAddr(GPIO,    0x08);
                PHYSetShortRAMAddr(GPIODIR, 0x08); 
            #endif
        ...
        /* Set interrupt mask, handle only Rx and Tx for now */
            ...
            PHYSetShortRAMAddr(INTMSK, 0xE6);//0b11100110);
        #else
            PHYSetShortRAMAddr(INTMSK, 0xF6);//0b11110110);  /* this is what we currently use */
        #endif
...
void PHYSetShortRAMAddr(BYTE address, BYTE value)
{
    ...
    /* allow the RF state machine to settle down before returning */
   	if ((address == PWRCTL) && (value == 0x00))
    {
		int localDelay;
		
		for (localDelay = 0; localDelay < 600; localDelay++)
			;                   /* do nothing */
	}
...
#define _DI1()		__asm__ volatile("disi #0x3FFF")
#define _EI1()		__asm__ volatile("disi #0")
    ...
    void __ISR(_EXTERNAL_1_VECTOR, ipl6) _INT1Interrupt(void)
#else
    void _ISR __attribute__((interrupt, auto_psv)) _INT1Interrupt(void)
#endif
{
    ...
    if(RFIF)
    {
        if(RFIE)
        {
            ...
            /* Clear interrupt only after checking it */
            RFIF = 0;	
            ...
            if (CheckInterrupt & 0x01)
            {
                ...
                if( (TxStatus & 0x21) == 0 )
                {
                    ...
                    if(( currentPacket.info.bits.RX_association && (Pending & 0x04 )) ||
            ...
            if (CheckInterrupt & 0x08)
            {
                ...
                //static BYTE rc = 0;
						...
						#define BUFFER_CAST WORD
                        WORD RxBytesRemaining;
                        WORD OldRxWrite;
                #else
                ...
                if(w.Val < RxBytesRemaining)
                {
                    ...
                    if(RxWrite==(MBUFFER_CAST)RX_BUFFER_SIZE)
                    {
                        RxWrite = 0;
                    }
                    ...
                    while(w.Val--)
                    {
                        ...
                        if(count==0)
                        {
                            ...
                            mfc.word.byte.LB=RxBuffer[RxWrite];
                        ...
                        else if(count==2)
                        {
                            ...
                            if(ack_status.bits.b0)
                            {
                                    ...
                                    (RxBuffer[RxWrite] == currentPacket.sequenceNumber))
                                {
                                    ...
                                    macTasksPending.bits.bSendUpMACConfirm = 1;
                                    pendingAckFrameControl.word = mfc.word;
                                }
                                RxWrite = OldRxWrite;
