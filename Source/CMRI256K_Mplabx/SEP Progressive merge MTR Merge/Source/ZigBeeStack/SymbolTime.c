...
void InitSymbolTimer()
{
        ...
        T2CONbits.TON = 1;
    #elif defined(__C32__) && defined(__PIC32MX__)
        T2CON = 0x0;
        T3CON = 0x0;
        T2CON= 0x78;    //1:256 prescale, timer2, timer3 forms 32-bit timer
        TMR2 = 0x0;
        PR2 = 0xFFFFFFFF;
        T2CONbits.TON = 1;
    #else
...
DWORD TickGetDiff(TICK a, TICK b)
{
    TICK diff;
    if (a.Val >= b.Val)
    {
    ...
    {
        diff.Val = 0xFFFFFFFF - b.Val + a.Val;
        return (diff.Val);
    }
}
