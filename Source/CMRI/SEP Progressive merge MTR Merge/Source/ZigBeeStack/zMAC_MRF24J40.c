...
unsigned char MyCurrentChannel = 0x00;
...
volatile CURRENT_PACKET                      currentPacket;
...
volatile MAC_TASKS_PENDING                   macTasksPending;
...
#if (I_SUPPORT_RANGE_TESTING == 0x01)
    extern BYTE        SuppressBeaconTransmission;
#endif // #if (I_SUPPORT_RANGE_TESTING == 0x01)

BYTE InterruptRoutineSet = 0;
...
{
    BYTE macData;
    BYTE* temp = params.PD_DATA_indication.psdu;
    params.PD_DATA_indication.psduLength--;
    macData = (*temp);
    params.PD_DATA_indication.psdu++;
    return macData;
}
...
void MACEnable (void)
{
    ...
    BYTE i;
    WORD validitykey = MAC_ADDRESS_VALID;
    ...
    PHYSetShortRAMAddr(PWRCTL,0x00);
        ...
        PutMACAddressValidityKey(&validitykey);
    #endif
}
...
void MACInit(void)
{
    ...
    if(NOW_I_AM_A_FFD()){//#if defined(I_AM_FFD)
        macStatus.Val = 0;
    }//#endif
...
ZIGBEE_PRIMITIVE MACTasks(ZIGBEE_PRIMITIVE inputPrimitive)
{
    ...
    if(inputPrimitive == NO_PRIMITIVE)
    {
        ...
        if (macTasksPending.bits.bSendUpMACConfirm)
        {
            ...
            frameControl.word = pendingAckFrameControl.word;
        ...
        if(macTasksPending.bits.packetPendingAck == 1)
        {
            if (TxStat.finished)
            {
                ...
                if (TxStat.success)
                {
                    ...
                    #endif
					if(MyCommandFlag.ACK == ACK_PENDING && MyCommandFlag.DataSent == TRUE)
					{
                ...
                else
                {
                    ...
                    ZigBeeUnblockTx();
                        ...
                        {
                            /* End Device failed to transmit data Continuously, try to join other parent */
                            /*params.NLME_JOIN_request.ScanDuration     = 8;
                            params.NLME_JOIN_request.ScanChannels.Val = current_SAS.spas.ChannelMask.Val;
                            params.NLME_JOIN_request.RejoinNetwork    = 0x02;
                            params.NLME_JOIN_request.SecurityEnable    = FALSE;*/

                            /* Build the capability information for NLME_JOIN_REQUEST */
                            ...
                            /* Try to join network which we have already joined */
                            /*BYTE i;
                            for (i = 0; i < 8; i++)
                            {
                            ...
                            /*The device has lost sync with the parent*/
                            if (NOW_I_AM_A_RFD()){//#ifdef I_AM_RFD
                                ZigBeeStatus.flags.bits.bLostSyncWithParent = 1;
                            }//#endif

                            ParentLinkRetryThreshold = 0x00;
        ...
        if(macTasksPending.bits.dataInBuffer == 1)
        {
            ...
            macTasksPending.bits.bDoNotSendMcpsDataConfirm = 1;
            goto SendTxBuffer;
        }
        ...
        if(macTasksPending.bits.scanInProgress == 1)
        {
            ...
            if(macTasksPending.bits.channelScanning == 0)
            {
                ...
                if(scanParams.Channels.bits.b0)
                {
                    ...
                    {
                        //#if defined(I_AM_FFD) || defined(INCLUDE_ED_SCAN) || I_SUPPORT_FREQUENCY_AGILITY == 1
                        case MAC_SCAN_ENERGY_DETECT:
                            scanParams.maxRSSI = 0;
                            break;
                        //#endif

                        //#if defined(INCLUDE_ACTIVE_SCAN)
                            case MAC_SCAN_ACTIVE_SCAN:
							//if(NOW_I_AM_A_FFD()){///*defined(I_AM_FFD) || */
                                if(TxStatus != 0x80)
                                {
                                    ...
                                    // beacon request cannot be secured
        ...
        switch(inputPrimitive)
        {
            case PD_DATA_indication:
            {
                 ...
                 
                {
                    ...
                    if ( macTasksPending.bits.bDoNotSendMcpsDataConfirm )
                    {
                        macTasksPending.bits.bDoNotSendMcpsDataConfirm = 0;
                    }
                    ...
                    frameControl.word.byte.LB = MACGet();
                    frameControl.word.byte.HB = MACGet();
                    ...
                    if(params.MCPS_DATA_indication.DstAddrMode == 0x02)
                    {
                    ...
                    else if(params.MCPS_DATA_indication.DstAddrMode == 0x03)
                    {
                        params.MCPS_DATA_indication.DstPANId.byte.LSB = MACGet();
                        params.MCPS_DATA_indication.DstPANId.byte.MSB = MACGet();
                    ...
                    else if(params.MCPS_DATA_indication.DstAddrMode == 0x01)
                    {
                        /* malformed packet, so safely reset radio */
                        MACDiscardRx();
                        PHY_WAKE = 1;
                    ...
                    */
                    /* Address Mode  - 16-bit short address */
                    if(params.MCPS_DATA_indication.SrcAddrMode == 0x02)
                    {
                        #ifdef ZCP_DEBUG
                    ...
                    /* Address Mode  - 64-bit extended address */
                    else if(params.MCPS_DATA_indication.SrcAddrMode == 0x03)
                    {
                        if(frameControl.bits.IntraPAN)
                        {
                            params.MCPS_DATA_indication.SrcPANId.byte.LSB = macPIB.macPANId.byte.LSB;
                    ...
                    else if(params.MCPS_DATA_indication.SrcAddrMode == 0x01)
                    {
                        /* malformed packet, so safely discard and reset the radio */
                        MACDiscardRx();
                        PHY_WAKE = 1;
                    ...
                    switch(frameControl.bits.FrameType)
                    {
                        case MAC_FRAME_TYPE_DATA:
                            ...
                            if( MACDataTransmission )
                            {
                                ...
                                if( i == 6 && lastByte == 0x16 )
                                {
                                    ...
                                    params.MCPS_DATA_request.SrcPANId.Val = MAC_DEFAULT_PANID;
                                    params.MCPS_DATA_request.DstPANId.Val = MAC_DEFAULT_PANID;
                        ...
                        case MAC_FRAME_TYPE_BEACON:
...

                            {
                                ...
                                for (i = 0; i < 8; i++)
                                {
                                    tempLongAddress.v[i] = params.MLME_BEACON_NOTIFY_indication.sdu[i + 3];
                                }
                            }
                            ...
                            if(macTasksPending.bits.scanInProgress == 1)
                            {
                                if((scanParams.ScanType == MAC_SCAN_ACTIVE_SCAN) || (scanParams.ScanType == MAC_SCAN_PASSIVE_SCAN))
                                {
                                    ...
                                    {
                                        LONG_ADDR extendPanID;
                                        p = (PAN_DESCRIPTOR*)scanParams.results;
                                        
                                        /* this block of code will allow the stack to 
                                         * reject lower LQI beacons from channels that are +/- 20MHz away
                                        */
                                        while(p != NULL)
                                        {
                                            
                                            extendPanID = p->ExtendedPANID;
                                            /* if beacons from same device and PAN are received then pick the higher LQI one */
                                            if( ( p->CoordPANId.Val   == params.MLME_BEACON_NOTIFY_indication.CoordPANId.Val) &&
                                                (p->CoordAddress.v[0] == params.MCPS_DATA_indication.SrcAddr.v[0]) &&
                                                (p->CoordAddress.v[1] == params.MCPS_DATA_indication.SrcAddr.v[1]) &&
                                                (!memcmp( (void*)&extendPanID, (void*)&tempLongAddress, 8) )
                                               )
                                            {
                                                if( p->LinkQuality >= params.MCPS_DATA_indication.mpduLinkQuality  )
                                                {
                                                    /* drop this lower LQI beacon */
                                                    return NO_PRIMITIVE;
                                                }
                                                else
                                                {
                                                    /* replace the LQI value in this PAN_DESCRIPTOR with the higher value */
                                            ...
                                            /* move to the next PAN descriptor in list and repeat comparison */
                                            p = p->next; 
                                        }
                                       
                                        /* reset things to the head of list and continue as before */
                                        p = (PAN_DESCRIPTOR*)scanParams.results; 
                                    ...
                                    if(p != NULL)
                                    {
                                        ...
                                        /* Ensure that this works with C30 and C32 compilers structure alignments */
                                        p->LinkQuality = params.MCPS_DATA_indication.mpduLinkQuality;
...
ProcessACKPacket:
                                    ZigBeeUnblockTx();
                                    macTasksPending.bits.packetPendingAck = 0;
                                    //merged #if !defined(I_AM_COORDINATOR)
                                        if((NOW_I_AM_NOT_A_CORDINATOR())&&currentPacket.info.bits.association == 1)
                                        {
                                            macTasksPending.bits.associationPending = 1;
                                            /* have aResponseWaitTime to get a response */
                                            currentPacket.startTime = TickGet();
                                        }
                                        else if(((NOW_I_AM_NOT_A_CORDINATOR())&&(currentPacket.info.bits.data_request == 1))
                                        		||((NOW_I_AM_A_CORDINATOR()) &&(currentPacket.info.bits.data_request == 1))
                                        		)
                                    //#else
                        ...
                        case MAC_FRAME_TYPE_CMD:
                            {
                                ...
                                switch(command)
                                {
                                    case ASSOCIATION_REQUEST:
                                        {
                                            ...
                                            /* preserve the source address to prevent overwriting */
                                            for(i = 0; i < 8; i++)
                                            {
                                              tempAddr.v[i] = params.MCPS_DATA_indication.SrcAddr.LongAddr.v[i];
                                            }
                                            
                                            /* ensure that device address is properly placed here */
                                            for(i = 0; i < 8; i++)
                                            {
                                              params.MLME_ASSOCIATE_indication.DeviceAddress.v[i] =  tempAddr.v[i];
                                            }
                                    ...
                                    case BEACON_REQUEST:
                                        ...
                                        // This added for range testing. It is difficult to keep device out reach to each other.'
                                        // By having this condition, we can suppress the beacon transmission. If we do this, then
                                        // the receiving device will not get the beacon from this device. This will help in not
                                        // creating the neighbor table which means the device is not in my proximity.
                                        #if (I_SUPPORT_RANGE_TESTING == 0x01)
                                            if ( SuppressBeaconTransmission )
                                            {
                                                return NO_PRIMITIVE;
                                            }
                                        #endif // #if (I_SUPPORT_RANGE_TESTING == 0x01)
                                        ...
                                        if(macStatus.bits.allowBeacon)
                                        {
                                            ...
                                            /* For beacon frames particularly, the destination PANID is not yet established so make it Zero */
                                            params.MCPS_DATA_request.DstPANId.byte.MSB  = 0x00;
                                            params.MCPS_DATA_request.DstPANId.byte.LSB  = 0x00;
                                    ...
                                    case DATA_REQUEST:
                                    {
                                        BYTE* packet = NULL;
                                        BYTE i,k;
                                        BYTE indirectDataExists = FALSE;

                                        if (InterruptRoutineSet)
                                        {
                                        ...
                                        for(i=0;i<(sizeof(macIndirectBuffers)/sizeof(MAC_INDIRECT_BUFFER));i++)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if(params.MCPS_DATA_indication.SrcAddrMode == 0x03 ) /* long addressing in the data request so we are looking for a packet with long addressing */
                                                {
                                                    if((k & 0x0C) == 0x0C)
                                                    {
                                                        BYTE i;
                                        ...
                                        if(addressMatch.Val == TRUE)
                                        {
                                            indirectDataExists = FALSE; //Found a valid entry. So set it back to FALSE.
                                            if(ZigBeeReady())
                                            {
                                                macTasksPending.bits.packetPendingAck = 0;
                                        ...
                                        {
                                            if (indirectDataExists)
                                            {
                                                indirectDataExists = FALSE; //Set it back to FALSE
                                                ...
                                                if(params.MCPS_DATA_indication.SrcAddrMode == 0x02 ||
                                                    params.MCPS_DATA_indication.SrcAddrMode == 0x03 )
                                                {
                                                #ifdef ZCP_DEBUG
            ...
            case MCPS_DATA_request:
            {
                ...
                if( TxBuffer[TxHeader+2] & 0x02 )   // security on
                {
                    ...
                    if( GetActiveNetworkKey(&ActiveNetworkKey))
                    {
                        ...
                        (TX_HEADER_START-TxHeader), &ActiveNetworkKey, TRUE))
                        {
                            ZigBeeUnblockTx();
...
MCPS_DATA_request_label_skip_init:
                {
                    ...
                    if(TxHeader < TxData)
                    {
                        ...
                        ZigBeeUnblockTx();
                        params.MCPS_DATA_confirm.status = FRAME_TOO_LONG;
...
SendTxBuffer:
                        {   //need this bracket for the #ifdef with the hanging else
...

                            {
                                ...
                                {
                                    TNow=TickGet();
                                    if(TickGetDiff(TNow,TStart)>=ONE_SECOND/200)
                                    {
                                        TxStat.finished = 1;
                                    }
...
//left alone as the following code is comment #if defined(I_AM_COORDINATOR) || defined(I_AM_ROUTER)
...
//#endif
                            ...
                            if(params.MCPS_DATA_request.TxOptions.bits.acknowledged_transmission != 1)
                            {
                                ...
                                // This is added because of a bug.
                                // The bug description : When unicast data tx is made to a resetted device,
                                // after any broadcast tranmission is done, Data Confirm is sent with Status as SUCCESS.
                                // Fixes : TxStat.success variable should explicitly set to ZERO.

                                TxStat.success = 0;
                                //TxStat.finished = 0;
                                //macTasksPending.bits.bSendUpMACConfirm = 0; // Is this required?
            ...
            case MLME_ASSOCIATE_response:
            {
                ...
                TxBuffer[TxData++]=params.MLME_ASSOCIATE_response.status;
                    ...
                    {
                        params.MCPS_DATA_request.TxOptions.Val = 0x0D;
                    }
                    else
                #endif
                {
                    params.MCPS_DATA_request.TxOptions.Val = 0x05;
                }
                ...
                /* Copy over DstAddress to ensure no compiler alignment issues */
                {
                    BYTE i;
                    
                    for(i=0; i < 8; i++)
                    {
                         params.MCPS_DATA_request.DstAddr.LongAddr.v[i] = currentPacket.DstAddr.v[i];
                    }
                }
            ...
            if(NOW_I_AM_NOT_A_CORDINATOR()){
            ...
            #endif
                {
                    ...
                    {
                        params.MCPS_DATA_request.TxOptions.Val = 0x09;//0b00001001;
                    }
                    else
                #endif
                    {
            ...
            case MLME_ORPHAN_response:
            {
                ...
                {
                    params.MCPS_DATA_request.TxOptions.Val = 0x09;
                }
                else
                {
                    params.MCPS_DATA_request.TxOptions.Val = 0x01;
...
MLME_SCAN_request_INVALID_PARAMETER:
                ...
                {
                    case MAC_SCAN_ENERGY_DETECT:
                        if(NOW_I_AM_A_FFD() || I_SUPPORT_FREQUENCY_AGILITY == 1){//#if defined(I_AM_FFD) || defined(INCLUDE_ED_SCAN) || I_SUPPORT_FREQUENCY_AGILITY == 1
							/* turn off auto mode while energy scanning */
                                ...
                                PHYSetLongRAMAddr(TESTMODE, 0x08);
                                    ...
                                    PHYSetShortRAMAddr(GPIO,    0x0C);        // Enable LNA
                                #else
                            ...
                            if(scanParams.results == NULL)
                            {
                                ...
                                #endif
                                params.MLME_SCAN_confirm.status = SUCCESS;
                                params.MLME_SCAN_confirm.UnscannedChannels = \
                                    params.MLME_SCAN_request.ScanChannels;
            ...
            case MLME_START_request:
            {
                ...
                if(params.MLME_START_request.fields.bits.PANCoordinator)
                {
					...
					
					{
						...
						//channel number for App layer nimish
						MyCurrentChannel = phyPIB.phyCurrentChannel;
            ...
            if(NOW_I_AM_NOT_A_CORDINATOR()){
                ...
                {
                    params.MCPS_DATA_request.TxOptions.Val = 0x01; //0b00000001;
                }
                ...
                /* Explicitly adding destination address to ensure this works with all C30 and C32 versions */
                params.MCPS_DATA_request.DstAddr.ShortAddr.Val = params.MLME_POLL_request.CoordAddress.ShortAddr.Val;

                //#ifdef I_AM_COORDINATOR
                if (((NOW_I_AM_A_CORDINATOR())&&!ZigBeeStatus.flags.bits.bNetworkFormed)
                || //#else
            #ifdef ZCP_DEBUG
                ((NOW_I_AM_NOT_A_CORDINATOR())&&(!ZigBeeStatus.flags.bits.bNetworkJoined && !ZigBeeStatus.flags.bits.rejoin || bDRLong))
            #else
                ((NOW_I_AM_NOT_A_CORDINATOR())&&(!ZigBeeStatus.flags.bits.bNetworkJoined && !ZigBeeStatus.flags.bits.rejoin))
            #endif
                )//#endif
                {
                    ZigBeeStatus.flags.bits.rejoin = 0;
                    #ifdef ZCP_DEBUG
                        bDRLong = FALSE;
                    #endif
                    params.MCPS_DATA_request.SrcAddrMode = 0x03;
