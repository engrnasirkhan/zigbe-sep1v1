...
NEIGHBOR_TABLE_INFO         currentNeighborTableInfo;               // Info about the neighbor table and the node's children.
    ...
    LINK_STATUS_RECORD_INFO     RAMNeighborTable[MAX_NEIGHBORS];
    LINK_STATUS_RECORD_INFO     *pRAMNeighborTable = RAMNeighborTable;
...
// Security Information
    ...
    NETWORK_KEY_INFO    currentNetworkKeyInfo;
        ...
        WORD networkKeyInfo;
        /* allow nwk security frame counters to be saved/restored */
 		WORD outgoingFrameCounterIndex;
    #else
        ROM BYTE nwkActiveKeyNumber;
        ROM NETWORK_KEY_INFO    networkKeyInfo[NUM_NWK_KEYS];
 //left alone as below is comment       #if defined(I_AM_COORDINATOR) || defined(I_AM_TRUST_CENTER)
...
//          ROM LONG_ADDR trustCenterLongAddr;
    ...
    #ifdef USE_EXTERNAL_NVM
        WORD appLinkKeyTable;
    #else
        ROM APS_KEY_PAIR_DESCRIPTOR appLinkKeyTable[MAX_APPLICATION_LINK_KEY_SUPPORTED] = {0x00};
    #endif
#endif
...
//        #ifdef USE_EXTERNAL_NVM
            WORD TCLinkKeyTable;
//        #else
//            ROM TC_LINK_KEY_TABLE TCLinkKeyTable[MAX_TC_LINK_KEY_SUPPORTED];
//       #endif
//    #endif
//#endif

//Commissiong Information
STARTUP_ATTRIBUTE_SET      current_SAS;       // RAM copy of current SAS.
#ifdef USE_EXTERNAL_NVM
    WORD default_SAS;
#else
    ROM STARTUP_ATTRIBUTE_SET default_SAS;
#endif
#ifdef I_SUPPORT_COMMISSIONING
    #ifdef USE_EXTERNAL_NVM
        WORD Commissioned_SAS;
        WORD activeSASIndex;
    #else
        ROM STARTUP_ATTRIBUTE_SET Commissioned_SAS[MAX_SAS_TABLE_SIZE];
        ROM BYTE activeSASIndex;
    #endif
#endif
...
//******************************************************************************
 ...
 /**********************************************************************
   ...
   Description:
     This routine allocates chunks of nonvolatile memory for all the
     nonvolatile tables.If there is not enough space,a non-zero value is
     returned.Otherwise, 0 is returned.
   Conditions:
     None
   Return:
     This function returns either '0'(Success) or '1'(Not Enough Space
     Available).
   Example:
     <code>
   ...
   Remarks:
     ...
     This routine needs to be called before attempting to read or write
     nonvolatile tables when external storage is used.                  
   **********************************************************************/
 ...
 ********************************************************************/
    ...
    {
        static WORD nextEEPosition = sizeof(MSDCL_Commission)+1;
...
void MemReverseCopy( BYTE *psrc, BYTE *pdestination, BYTE length )
{
    BYTE count, destIndex = 0;
    for( count = length; count != 0; count-- )
    {
        *( pdestination + destIndex ) = *( psrc + ( count - 1 ) );
        destIndex++;
    }
}
