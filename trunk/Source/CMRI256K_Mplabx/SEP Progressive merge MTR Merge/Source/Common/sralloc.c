...
BYTE CountCurrentMemoryAllocated;
...
unsigned char * NEAR SRAMalloc(NEAR unsigned char nBytes)
{
        ...
        memoryAllocated = (unsigned char *)malloc((size_t)nBytes);
        if( memoryAllocated != NULL)
        {
            CountCurrentMemoryAllocated++;
        }
...
/*********************************************************************
 * Function:        BYTE CurrentMemoryUse
 *
 * PreCondition:    None
 *
 *
 * Input:           None
 *
 * Output:          Returns the number of times memory allocated that
 *                  are not freed.
 *
 * Side Effects:
 *
 * Overview:        This function returns the number of times memory allocated
 *                  and not freed yet.This function can be used to find any
 *                        memory leak in the system. Used for diagnostic purpose

 ********************************************************************/
BYTE CurrentMemoryUse(void )
{
    return CountCurrentMemoryAllocated;
}
 ...
 * Input:           None
 *
 * Output:          Resets the memory usage counter to 1
 *
 * Side Effects:
 *
 * Overview:        This function resets the memory usge count to
 *                   1 to virtually remove memory leak.
 *                   Used for diagnostic purpose.

 ********************************************************************/
void ResetMemoryCount(void )
{
    /*This is initialized to 3 because 3 buffers are held up.
    one for UART, one for the Request command, and one for
    the Response/Confirm command.*/
    CountCurrentMemoryAllocated = 3;
}
/*********************************************************************
