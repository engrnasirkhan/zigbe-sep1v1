...
typedef struct _APS_STATUS
{
    union _APS_STATUS_flags
    {
        ...
        struct _APS_STATUS_bits
        {
            ...
            BYTE    bLoopBackData               : 1;
            BYTE    bFreeAllocatedAsdu          : 1;
            #if (I_SUPPORT_FRAGMENTATION == 1)
                BYTE bFragmentationInProgress   : 1;        // Specifies fragmentation is under progress.
                BYTE bWaitingForFragmentDataConfirm : 1;    // Specifies block has been transmitted and waiting for confirm from lower layer.
                BYTE bPendingFragmentDataIndication : 1;    // Specifies all the blocks has been received.
            #endif //(I_SUPPORT_FRAGMENTATION == 1)
            #if (I_SUPPORT_LINK_KEY == 1)
                BYTE    bSaveOutgoingFrameCounter   : 1;    // Save the OutgoingFrameCounter
