...
// MAC Address
    ...
    extern WORD                         macLongAddrValidityKey;
#else
...
/* Keep a RAM shadow of Neighbor Table to minimize EEPROM writes */
#if ( I_SUPPORT_LINK_STATUS_COMMANDS == 0x01 )
extern LINK_STATUS_RECORD_INFO     RAMNeighborTable[MAX_NEIGHBORS];
extern LINK_STATUS_RECORD_INFO     *pRAMNeighborTable;
...
// Security Information
    ...
    extern NETWORK_KEY_INFO             currentNetworkKeyInfo;
        ...
        extern WORD						outgoingFrameCounterIndex;
        extern WORD                     incomingFrameCounterIndex;
...
/*        #if !(defined(I_AM_COORDINATOR) || defined(I_AM_TRUST_CENTER))
    ...
    #ifdef USE_EXTERNAL_NVM
        extern  WORD appLinkKeyTable;
    #else
        extern  ROM APS_KEY_PAIR_DESCRIPTOR appLinkKeyTable[MAX_APPLICATION_LINK_KEY_SUPPORTED];
   #endif
#endif
...
//        #ifdef USE_EXTERNAL_NVM
            extern  WORD TCLinkKeyTable;
//        #else
//            extern  ROM TC_LINK_KEY_TABLE TCLinkKeyTable[MAX_TC_LINK_KEY_SUPPORTED];
//       #endif
//    #endif
//#endif
...
// Descriptors
#ifdef USE_EXTERNAL_NVM
    extern WORD                         nodeDescriptorValidityKey;
    ...
    extern  NODE_POWER_DESCRIPTOR    Config_Power_Descriptor;
...
#ifdef USE_EXTERNAL_NVM
   extern WORD persistencePIB;
#endif

//Commissiong Information
extern STARTUP_ATTRIBUTE_SET      current_SAS;       // RAM copy of current SAS.
#ifdef USE_EXTERNAL_NVM
    extern WORD default_SAS;
#else
    extern ROM STARTUP_ATTRIBUTE_SET default_SAS;
#endif
#ifdef I_SUPPORT_COMMISSIONING
    #ifdef USE_EXTERNAL_NVM
        extern WORD Commissioned_SAS;
        extern WORD activeSASIndex;
    #else
        extern ROM STARTUP_ATTRIBUTE_SET Commissioned_SAS[MAX_SAS_TABLE_SIZE];
        extern ROM BYTE activeSASIndex;
    #endif
#endif

#if (I_SUPPORT_REPORTING == 1)
    extern WORD ReportingBackgroundIndex;
    extern WORD ReportCfgTableIndex;
#endif
/*******************************************************************************
...
*******************************************************************************/
    ...
    #define  NODE_DESCRIPTOR_VALID 0xA35C
    #define  MAC_ADDRESS_VALID     0xA35D
    #if defined(USE_EXTERNAL_NVM) && defined(STORE_MAC_EXTERNAL)
        #define GetMACAddressValidityKey(x)     NVMRead( (BYTE *)x, macLongAddrValidityKey, sizeof(WORD))
        #define PutMACAddressValidityKey(x)     NVMWrite( macLongAddrValidityKey, (BYTE *)x, sizeof(WORD))
    ...
    #define GetNodeDescriptorValidity(x)    NVMRead( (BYTE *)x, nodeDescriptorValidityKey, sizeof(WORD))
    #define PutNodeDescriptorValidity(x)    NVMWrite( nodeDescriptorValidityKey, (BYTE *)x, sizeof(WORD))
...
//    #define PutTrustCenterAddress( x )      NVMWrite( trustCenterLongAddr, (BYTE*)x, sizeof(LONG_ADDR) )
    ...
    #define GetLinkKey(x, y )               NVMRead( (BYTE *)x, y, sizeof(APS_KEY_PAIR_DESCRIPTOR) )
    #define PutLinkKey(x, y )               NVMWrite( x, (BYTE*)y, sizeof(APS_KEY_PAIR_DESCRIPTOR) )

//    #if I_SUPPORT_LINK_KEY == 1
//        #if I_SUPPORT_MULTIPLE_TC_LINK_KEY == 1
            #define GetTCLinkKey(x, y )               NVMRead( (BYTE *)x, y, sizeof(TC_LINK_KEY_TABLE) )
            #define PutTCLinkKey(x, y )               NVMWrite( x, (BYTE*)y, sizeof(TC_LINK_KEY_TABLE) )
//        #endif
//    #endif
    ...
    /* Save/restore OutgoingFrameCounters */    
    #define PutOutgoingFrameCount(x)		NVMWrite(outgoingFrameCounterIndex, (BYTE *)x, (sizeof(DWORD_VAL)*2))
    #define GetOutgoingFrameCount(x)		NVMRead ((BYTE *)x, outgoingFrameCounterIndex, (sizeof(DWORD_VAL)*2))
    ...
    #ifdef I_SUPPORT_COMMISSIONING
        #define PutActiveSASIndex(x)        NVMWrite(activeSASIndex, (BYTE *)x, sizeof(BYTE))
        #define GetActiveSASIndex(x)        NVMRead((BYTE *)x, activeSASIndex, sizeof(BYTE))
    #endif
    #define PutSAS(x,y)                 NVMWrite(x, (BYTE *)y, sizeof(STARTUP_ATTRIBUTE_SET))
    #define GetSAS(x,y)                 NVMRead((BYTE *)x, y, sizeof(STARTUP_ATTRIBUTE_SET))
    #define PutAttributeStorage(x, y, z)    NVMWrite(x, y, z)
    ...
    #define ProfileGetSimpleDesc(p,i)       (NOW_I_AM_A_ROUTER()?NVMRead( (BYTE *)p, (ROM void*)&(Config_Simple_Descriptors_MTR[i]), sizeof(NODE_SIMPLE_DESCRIPTOR)):NVMRead( (BYTE *)p, (ROM void*)&(Config_Simple_Descriptors_ESP[i]), sizeof(NODE_SIMPLE_DESCRIPTOR)))
...
//    #define PutTrustCenterAddress( x )      NVMWrite( (NVM_ADDR *)&trustCenterLongAddr, (BYTE *)x, sizeof(LONG_ADDR) )
    ...
    #ifdef I_SUPPORT_COMMISSIONING
        #define PutActiveSASIndex(x)        NVMWrite((NVM_ADDR *)&activeSASIndex, (BYTE *)x, sizeof(BYTE))
        #define GetActiveSASIndex(x)        NVMRead((BYTE *)x, (ROM void *)&activeSASIndex, sizeof(BYTE))
    #endif
    #define PutSAS(x,y)                 NVMWrite((NVM_ADDR *)x, (BYTE *)y, sizeof(STARTUP_ATTRIBUTE_SET))
    #define GetSAS(x,y)                 NVMRead((BYTE *)x, (ROM void *)y, sizeof(STARTUP_ATTRIBUTE_SET))
#endif
...
/******************************************************************************
 * Function:        void MemReverseCopy( BYTE *psrc, BYTE *pdestination, BYTE length )
 *
 * PreCondition:    None
 *
 * Input:           *psrc : pointer to source memory location
 *                  *pdestination : pointer to destination memory location
 ...
 * Side Effects:    None
 *
 * Overview:        Copies the given number of bytes from source to destination
 *
 * Note:            None
 *****************************************************************************/
void MemReverseCopy
(
    ...
    extern void EE_SPIPut( BYTE x );
    extern BYTE EE_SPIGet(void);
#endif
